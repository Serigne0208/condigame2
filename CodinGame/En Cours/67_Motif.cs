    ...
    class _67_Motif
    {
        public static int[] Solve(int imageWidth, int imageHeight, string[] image, int patternWidth, int patternHeight, string[] pattern)
        {
                ...
                (patternWidth <= imageWidth) && (patternHeight <= imageHeight))
            {
                ...
                for (int img_x = 0; (pattern_not_found && (img_x <= (imageWidth - patternWidth))); img_x++)
                { // <- check if img_x == (imageWidth - patternWidth) or not
                    for (int img_y = 0; (pattern_not_found && (img_y <= (imageHeight - patternHeight))); img_y++)
                    { // <- check if img_y == (imageHeight - patternHeight) or not
